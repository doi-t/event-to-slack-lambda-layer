locals {
  ...
  # A package directory that manages all source codes and its dependencies 
...
resource "null_resource" "create_lambda_package" {
  ...
  provisioner "local-exec" {
    command = "${path.module}/create-lambda-package.sh ${var.package_name} ${var.python_version} ${var.source_dir} ${local.package_source_dir} ${local.build_dir}"
  }
}
